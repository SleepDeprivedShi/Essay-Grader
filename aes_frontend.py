...
class EssayGrader(QWidget):
    ...
    def submit_essay(self):
        ...
        content_score = self.backend.evaluate_content(reference_essay, student_essay)
        organization_score = self.backend.evaluate_organization(reference_essay, student_essay)
        grammar_mechanics_score = self.backend.evaluate_grammar_mechanics(student_essay)
        word_choice_score = self.backend.evaluate_word_choice(student_essay)
        total_score = self.backend.grade_essay(reference_essay, student_essay)
